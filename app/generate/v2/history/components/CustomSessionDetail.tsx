			...
			<div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-white/6">
				<div className="min-w-0">
					<div className="flex items-center gap-2 mb-0.5">
						...
						<span className={`inline-flex items-center gap-1 rounded-full border px-2 py-px text-[10px] font-semibold capitalize ${colors.pill}`}>
				...
				<div className="px-4 pb-3 grid grid-cols-2 gap-3">
					{meme.videoUrl && (
						<div>
							...
							<div className="flex gap-1.5 mt-1.5">
								<a href={meme.videoUrl} target="_blank" rel="noopener noreferrer"
					...
					{meme.highResUrl && (
						<div>
							...
							<div className="flex gap-1.5 mt-1.5">
								<a href={meme.highResUrl} target="_blank" rel="noopener noreferrer"
									className="flex-1 flex items-center justify-center gap-1 rounded-lg border border-white/8 py-1 text-[10px] text-white/40 hover:text-white/80 hover:bg-white/[0.05] transition-all">
									<ExternalLink className="size-2.5" /> View
								</a>
								<a href={meme.highResUrl} download
									className="flex-1 flex items-center justify-center gap-1 rounded-lg border border-white/8 py-1 text-[10px] text-white/40 hover:text-white/80 hover:bg-white/[0.05] transition-all">
									<Download className="size-2.5" /> Save
								</a>
							</div>
...
export const CustomSessionDetail: React.FC<CustomSessionDetailProps> = memo(({ sessionId }) => {
	...
	const sessionMemes = useMemo(
		() => customMemes.filter((m) => m.memeId === sessionId),
		[customMemes, sessionId]
	);
					...
					<div>
						...
						<div className="mt-2 rounded-lg border border-white/6 bg-white/[0.015] px-3 py-2 space-y-1">
							<div className="flex items-start gap-2">
								<span className="shrink-0 text-[9px] font-bold uppercase tracking-widest text-violet-400/60 w-24">nodeOrSessionId</span>
								<span className="text-[10px] font-mono text-white/40 break-all">{currentSourceId || '—'}</span>
							</div>
							<div className="flex items-start gap-2">
								<span className="shrink-0 text-[9px] font-bold uppercase tracking-widest text-emerald-400/60 w-24">sessionId</span>
								<span className="text-[10px] font-mono text-white/40 break-all">{sessionId}</span>
							</div>
							<p className="text-[9px] text-white/20 pt-0.5">nodeId &amp; memeId are returned/set by the API after creation</p>
						</div>
					</div>
					...
					>
						...
						<span className="relative flex items-center justify-center gap-2">
							{loading
								...
								: <><Sparkles className="size-4" /> Create Meme from {sources.find((s) => s.id === currentSourceId)?.label ?? 'Source'}</>
							}
						</span>
					</button>
				</div>
